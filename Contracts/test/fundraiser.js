...
const Fundraiser = artifacts.require("./CampaignTokenFundraiser.sol");
const TestableFundraiser = artifacts.require("./testing/TestableFundraiser.sol");
...
contract('Fundraiser', function (accounts) {
    ...
    describe('contract construction', function () {
        ...
        it('should throw an error if no beneficiary is set', async function () {
            await expectThrow(TestableFundraiser.new(0, initialConversionRate, oneDayBefore, oneDayAfter, hardCap, minimumContribution, individualLimit));
        });

        it('should have all parameters set as expected', async function () {
            token = await TestableFundraiser.new(accounts[1], initialConversionRate, oneDayBefore, oneDayAfter, hardCap, minimumContribution, individualLimit);
    ...
    describe('before the fundraiser begin', function () {
        beforeEach(async function () {
            token = await TestableFundraiser.new(accounts[0], initialConversionRate, oneDayAfter, twoDaysAfter, hardCap, minimumContribution, individualLimit);
        });
    ...
    describe('during the fundraiser', function () {
        beforeEach(async function () {
            token = await TestableFundraiser.new(accounts[0], initialConversionRate, oneDayBefore, oneDayAfter, hardCap, minimumContribution, individualLimit);
        });
        ...
        it('should transfer the correct amount of ether to the beneficiary', async function () {
            token = await TestableFundraiser.new(accounts[9], initialConversionRate, oneDayBefore, oneDayAfter, hardCap, minimumContribution, individualLimit);
    ...
    describe('funds receive during the fundraiser', function () {
        ...
        it('should allow to buy tokens with higher or equal than the minimal contribution', async function () {
            token = await TestableFundraiser.new(accounts[0], initialConversionRate, oneDayBefore, oneDayAfter, hardCap, web3.toWei("0.1", "kwei"), individualLimit);
        ...
        it('should not allow to buy tokens with lower than the minimal contribution', async function () {
            token = await TestableFundraiser.new(accounts[0], initialConversionRate, oneDayBefore, oneDayAfter, hardCap, web3.toWei("0.1", "ether"), individualLimit);
    ...
    describe('second before and after the end date fundraiser', function () {
        beforeEach(async function () {
            updateTimestamps();
            token = await TestableFundraiser.new(accounts[0], initialConversionRate, oneDayBefore, now + 5, hardCap, minimumContribution, individualLimit);
            updateTimestamps()
        });
