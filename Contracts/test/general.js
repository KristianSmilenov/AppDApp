...
const CampaignToken = artifacts.require("./CampaignToken.sol");

contract('CampaignToken (general)', function (accounts) {
  let token;

  it("should have all parameters set correctly", async function () {
    token = await CampaignToken.new(0);

    let name = await token.name.call();
    assert.equal(name, 'Token', "incorrect name");

    let symbol = await token.symbol.call();
    assert.equal(symbol, 'TOKEN', "incorrect symbol");

    let decimals = await token.decimals.call();
    assert.equal(decimals, 18, "decimals should match ETH decimals (wei)");
  });

  it("should have 10000 Tokens in the first account in the deployed contract", async function () {
    token = await CampaignToken.deployed();
  ...
  it("should put 10000 CampaignToken in owner account when new contract is created", async function () {
    token = await CampaignToken.new(10000);
  ...
  it("should send coin correctly", function () {
    ...
    return CampaignToken.deployed().then(function (instance) {
      token = instance;
